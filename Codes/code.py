...
def train(train_X, train_y, num_of_features, classes, layer_det, id, num_epochs=30, batch_size=100):
...
def test(dataset_name):
	...
	for i in range(3):
		...
		if (i==0):
			predictions=predict(dataset_X, (i+1))
		else:
			predictions=np.concatenate((predictions,predict(dataset_X, (i+1))),axis=1)
...
def splitting_and_feature_selection(dataset_name):
	...
	for i in range(3):
		X=train_X.iloc[:,(i)*num_features[i-1]:(i+1)*num_features[i]]
		X['class']=train_y
	...
	#*********** WRITE VALIDATION **********************
	for i in range(3):
		X=val_X.iloc[:,(i)*num_features[i-1]:(i+1)*num_features[i]]
		X['class']=val_y
...
def training_module(dataset_name):
	...
	for i in range(3):
		...
		train(train_X,train_y,num_of_features=num_features[i],classes=num_classes, layer_det=layer_det[i], id=(i+1), num_epochs=num_epochs[i], batch_size=batch_size[i])
	#***************************************************
...
def second_training(dataset_name):
	...
	train(train2_X,train2_y,num_of_features=num_classes*3,classes=num_classes, layer_det=layer_det[3], id=4, num_epochs=num_epochs[3], batch_size=batch_size[3])
	#***************************************************************
